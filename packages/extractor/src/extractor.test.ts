  ...
  describe("ContentExtractor", () => {
    it("should extract content successfully", async () => {
      const result = await extractor.extract(
        "data:text/html," + encodeURIComponent(sampleHtml),
      );
    ...
    it("should extract metadata", async () => {
      ...
      expect(result.content?.metadata.description).toBe(
    ...
    it("should handle empty content", async () => {
      ...
      const result = await extractor.extract(
        "data:text/html," + encodeURIComponent(emptyHtml),
      );
    ...
    it("should handle large content", async () => {
      ...
      const result = await extractor.extract(
        "data:text/html," + encodeURIComponent(largeContent),
      );
...
<body>
  ...
  describe("UrlNormalizer", () => {
    it("should normalize URLs", () => {
      const normalized = urlNormalizer.normalize(
    ...
    it("should extract domain", () => {
      expect(urlNormalizer.getDomain("https://sub.example.com/path")).toBe(
        "sub.example.com",
      );
    });
    ...
    it("should extract path", () => {
      expect(
        urlNormalizer.getPath("https://example.com/path/to/resource"),
      ).toBe("/path/to/resource");
    });

    it("should remove query parameters", () => {
      expect(
        urlNormalizer.removeQuery("https://example.com/path?param=value"),
      ).toBe("https://example.com/path");
    });

    it("should remove fragment", () => {
      expect(
        urlNormalizer.removeFragment("https://example.com/path#section"),
      ).toBe("https://example.com/path");
    });

    it("should compare URLs", () => {
      expect(
        urlNormalizer.areEqual(
          "https://example.com/",
    ...
    it("should convert relative to absolute", () => {
      expect(urlNormalizer.toAbsolute("/path", "https://example.com")).toBe(
  ...
  describe("ContentFilter", () => {
    it("should filter markdown content", () => {
      const filtered = contentFilter.filter(
    ...
    it("should filter HTML content", () => {
      const filtered = contentFilter.filterHtml(
    ...
    it("should check if content is worth keeping", () => {
      expect(
        contentFilter.isWorthKeeping(
          "This is a substantial piece of content with many words.",
        ),
      ).toBe(true);
  ...
  describe("Integration Tests", () => {
    ...
    it("should handle batch extraction", async () => {
      const urls = [
        "data:text/html," +
          encodeURIComponent("<html><body><h1>Test 1</h1></body></html>"),
        "data:text/html," +
          encodeURIComponent("<html><body><h1>Test 2</h1></body></html>"),
      ];
...
<body>
    ...
    <a href="link.html">Link</a>
      ...
      const result = await extractor.extract(
        "data:text/html," + encodeURIComponent(htmlWithAllFeatures),
      );
    ...
    it("should handle extraction options", async () => {
      ...
      const result = await extractorWithOptions.extract(
        "data:text/html," + encodeURIComponent(sampleHtml),
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should handle malformed HTML", async () => {
      const malformed = "<html><head><title>Test</head><body>";
      const result = await extractor.extract(
        "data:text/html," + encodeURIComponent(malformed),
      );
    ...
    it("should handle empty responses", async () => {
      ...
      const result = await extractor.extract(
        "data:text/html," + encodeURIComponent(empty),
      );
