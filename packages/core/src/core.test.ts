...
import { Validator, RequiredRule, TypeRule, StringRule, NumberRule, ArrayRule, ObjectRule, CustomRule, URLValidator, EmailValidator, ConfigValidator } from './validation';
...
describe('Core Utilities', () => {
  ...
  describe('PerformanceMonitor', () => {
    ...
    it('should measure asynchronous operations', async () => {
      const { result, metrics } = await monitor.measureAsync('async-test', async () => {
  ...
  describe('Validation System', () => {
    ...
    it('should validate strings', () => {
      validator.addRule('title', new StringRule({
        minLength: 5,
        maxLength: 50,
    ...
    it('should validate numbers', () => {
      validator.addRule('score', new NumberRule({
        min: 0,
        max: 100,
        integer: true,
    ...
    it('should validate arrays', () => {
      validator.addRule('tags', new ArrayRule({
        minLength: 1,
        maxLength: 10,
        unique: true,
    ...
    it('should validate objects', () => {
      validator.addRule('config', new ObjectRule({
        requiredFields: ['host', 'port'],
        message: 'Config must have host and port'
      }));

      const invalidResult = validator.validateField('config', { timeout: 5000 });
      expect(invalidResult.valid).toBe(false);

      const validResult = validator.validateField('config', { host: 'localhost', port: 3000 });
      expect(validResult.valid).toBe(true);
    });
  ...
  describe('Integration Tests', () => {
    ...
    it('should work together - validation and logging', () => {
      ...
      validator.addRule('email', new TypeRule('string', 'Email must be string'));
