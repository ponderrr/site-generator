...
import { PerformanceMonitor, measurePerformance, measureAsyncPerformance } from './performance';
import { Validator, RequiredRule, TypeRule, StringRule, NumberRule, ArrayRule, ObjectRule, CustomRule } from './validation';
...
describe('Extended Core Functionality Tests', () => {
  ...
  describe('Validation System - Complex Scenarios', () => {
    ...
    it('should validate nested objects', () => {
      validator.addRule('user.profile.name', new RequiredRule('Name is required'));
      validator.addRule('user.profile.email', new TypeRule('string', 'Email must be string'));
      validator.addRule('user.settings.preferences', new ArrayRule({
        minLength: 1,
        maxLength: 10,
      ...
      expect(validator.validate(validData)).toEqual({ valid: true, errors: [], warnings: [] });
      expect(validator.validate(invalidData).valid).toBe(false);
    });

    it('should handle custom validation rules', () => {
      validator.addRule('password', new CustomRule(
        (value: string) => value.length >= 8 && /[A-Z]/.test(value) && /[0-9]/.test(value),
        'Password must be 8+ chars with uppercase and number'
      ));

      expect(validator.validateField('password', 'weak')).toEqual({
        valid: false,
        errors: [{ field: 'password', message: 'Password must be 8+ chars with uppercase and number', code: 'CUSTOM_ERROR', value: 'weak' }],
        warnings: []
      });
    ...
    it('should validate arrays with complex rules', () => {
      validator.addRule('items', new ArrayRule({
        minLength: 2,
        maxLength: 5,
        itemRules: [
          new ObjectRule({
            ...
            customRules: {
              name: new CustomRule((val: string) => val.length > 0, 'Name cannot be empty'),
    ...
    it('should handle conditional validation', () => {
      ...
      validator.addRule('email', new CustomRule(
      ...
      expect(validator.validate({ type: 'user', email: 'invalid' }).valid).toBe(false);
      expect(validator.validate({ type: 'admin', email: 'no-email' }).valid).toBe(true);
    });
  });
  ...
  describe('ErrorHandler - Advanced Error Scenarios', () => {
    it('should handle network errors with retry', async () => {
      ...
      const networkOperation = async () => {
        attempts++;
        if (attempts <= 2) { // Succeed on the 3rd attempt (attempts = 3)
      ...
      const result = await defaultErrorHandler.handleWithRetry(networkOperation, context, {
        maxAttempts: 3,
        retryableErrors: ['NETWORK_ERROR', 'TIMEOUT', 'TEMPORARY_FAILURE', 'ECONNRESET']
      });
    ...
    it('should handle circuit breaker patterns', async () => {
      ...
      for (let i = 0; i < 6; i++) {
        try {
          await defaultErrorHandler.handleWithRetry(failingOperation, context, { maxAttempts: 1 });
        } catch (error) {
          // Expected to fail
        }
      }
      ...
      const testServiceBreaker = circuitStatus.find(cb => cb.component === 'test-service');
  ...
  describe('MetricsCollector - Advanced Analytics', () => {
    ...
    it('should handle high-frequency metrics', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        defaultMetricsCollector.record(`metric${i % 10}`, i, 'counter', { batch: Math.floor(i / 100) });
      }

      const summary = defaultMetricsCollector.getMetricsSummary();
      expect(summary.length).toBeGreaterThan(5);

      const aggregated = defaultMetricsCollector.getAggregatedMetrics('metric0', 'avg');
    ...
    it('should handle time-series analysis', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        defaultMetricsCollector.record('timeseries', Math.sin(i / 10) * 100, 'gauge', { iteration: i });
        await new Promise(resolve => setTimeout(resolve, 10));
      }

      const endTime = Date.now();
      const metrics = defaultMetricsCollector.getMetrics('timeseries', startTime);
    ...
    it('should handle complex aggregation scenarios', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        defaultMetricsCollector.record('performance', 100 + (i % 20), 'histogram', tags);
      }

      const byRegion = defaultMetricsCollector.getMetricsByTags({ region: 'us-east' });
      expect(byRegion.length).toBeGreaterThan(0);

      const aggregated = defaultMetricsCollector.getAggregatedMetrics('performance', 'avg', Date.now() - 3600000, { service: 'api' });
    ...
    it('should handle percentile calculations', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        defaultMetricsCollector.record('latency', value, 'histogram', { percentile_test: true });
      }

      const p50 = defaultMetricsCollector.getAggregatedMetrics('latency', 'p50');
      const p95 = defaultMetricsCollector.getAggregatedMetrics('latency', 'p95');
      const p99 = defaultMetricsCollector.getAggregatedMetrics('latency', 'p99');
  ...
  describe('Integration - Cross-Component Workflows', () => {
    it('should handle metrics collection with performance monitoring', async () => {
      ...
      const result = await monitor.measureAsyncPerformance(async () => {
        ...
        defaultMetricsCollector.record('operation.duration', 100, 'histogram', { operation: 'test' });
        defaultMetricsCollector.incrementCounter('operation.count', 1, { type: 'async' });
    ...
    it('should handle error handling with logging and metrics', async () => {
      ...
      const context = { operation: 'test-operation', component: 'integration-test' };
    ...
    it('should handle parallel processing with metrics', async () => {
      ...
      const tasks = Array.from({ length: 10 }, (_, i) => ({
        ...
        execute: async (data: any) => {
          ...
          defaultMetricsCollector.record('parallel.task', data.delay, 'histogram', { taskId: data.taskId });
          return data.taskId;
        }
      }));
