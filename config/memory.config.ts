...
export class RuntimeMemoryConfig {
   ...
   */
  getWorkerLimits() {
    const availableMemory = this.systemMemory * MEMORY_LIMITS.system.maxMemoryPercent;
    const memoryPerWorker = Math.floor(availableMemory / (this.cpuCount * 2)); // Conservative allocation
    
    return {
      maxOldGenerationSizeMb: Math.min(MEMORY_LIMITS.worker.maxOldGenerationSizeMb, memoryPerWorker / (1024 * 1024)),
      maxYoungGenerationSizeMb: MEMORY_LIMITS.worker.maxYoungGenerationSizeMb,
      maxMemoryUsage: Math.min(MEMORY_LIMITS.worker.maxMemoryUsage, memoryPerWorker),
    };
  }
  ...
  getMainProcessLimits() {
    ...
    return {
      maxOldSpaceSize: Math.min(MEMORY_LIMITS.main.maxOldSpaceSize, Math.floor(availableMemory / (1024 * 1024))),
      maxOldGenerationSizeMb: Math.min(MEMORY_LIMITS.main.maxOldGenerationSizeMb, Math.floor(availableMemory / (1024 * 1024))),
      maxMemoryUsage: Math.min(MEMORY_LIMITS.main.maxMemoryUsage, availableMemory),
    };
  }
  ...
  getCacheConfig() {
    const availableMemory = this.systemMemory * MEMORY_LIMITS.system.maxMemoryPercent;
    const cacheMemory = Math.min(MEMORY_LIMITS.cache.maxSize, availableMemory * 0.1); // Use 10% for cache
  ...
  validateConfiguration(config: {
    ...
    if (config.workerMemory) {
      ...
      if (config.workerMemory > maxWorkerMemory) {
        errors.push(`Worker memory (${config.workerMemory}) exceeds safe limit (${maxWorkerMemory})`);
      } else if (config.workerMemory > maxWorkerMemory * 0.8) {
        warnings.push(`Worker memory (${config.workerMemory}) is high, consider reducing`);
      }
    }
    ...
    if (config.mainMemory) {
      ...
      if (config.mainMemory > maxMainMemory) {
        errors.push(`Main process memory (${config.mainMemory}) exceeds safe limit (${maxMainMemory})`);
      } else if (config.mainMemory > maxMainMemory * 0.8) {
        warnings.push(`Main process memory (${config.mainMemory}) is high, consider reducing`);
      }
    }
    ...
    if (config.cacheMemory) {
      ...
      if (config.cacheMemory > maxCacheMemory) {
        errors.push(`Cache memory (${config.cacheMemory}) exceeds safe limit (${maxCacheMemory})`);
      }
    }
  ...
  getSystemInfo() {
    return {
      ...
      availableMemory: this.systemMemory * MEMORY_LIMITS.system.maxMemoryPercent,
      memoryWarningThreshold: this.systemMemory * MEMORY_LIMITS.system.warningThreshold,
      memoryCriticalThreshold: this.systemMemory * MEMORY_LIMITS.system.criticalThreshold,
    };
  }
}
...
export class MemoryMonitor {
   ...
   */
  startMonitoring(onWarning?: (usage: NodeJS.MemoryUsage) => void, onCritical?: (usage: NodeJS.MemoryUsage) => void) {
    this.monitorInterval = setInterval(() => {
      ...
      if (memoryPressure > this.thresholds.criticalThreshold && onCritical) {
        onCritical(usage);
      } else if (memoryPressure > this.thresholds.warningThreshold && onWarning) {
        onWarning(usage);
      }
